...
- 사용자·개발자 문서, 과거 검증 보고서, 릴리스 설명, 이슈·PR 안내를 한국어로 통일
- 한국어 문서 작성 원칙과 MIT 라이선스·측정 원본의 한국어 안내 추가

### 추가

- 승인된 원격 G7의 기존 모드를 복원하며 5VU 혼합·경로별 3회 비교를 재현하는 실서버 벤치마크 도구
- g7devops.com 공개 HTTPS에서 수집한 총 2,880건의 원시 결과, 서버 자원 측정값, 3회 중앙값 보고서
...
- 예약·CLI 아웃박스 복구가 현재 이벤트 토큰을 확인하고 완료된 장벽까지 해제하도록 수정; 더 최신 이벤트·제어면 재구축 장벽은 보존
- 일반 HIT·락 획득/대기 후 HIT와 저장 직전에 장벽 토큰·실행 세대·세대를 재확인해 동시 갱신·제어면 재설정 중 이전 응답 재사용 방지
...
- 120ms 지연 표시, 최소 표시시간 없음, 다크·모바일·동작 줄이기·접근성 지원
...
- HIT 전에도 원본과 같은 로그인 사용자 또는 공개 역할 읽기 권한을 재검사
...
- 공식 G7 7.0.9 커밋에 고정한 PHP 8.2/8.5 CI와 표준 array/Redis `PluginCacheDriver` 제어면·DB 임대 잠금 통합 검증

## [0.3.0-beta.1] - 2026-09-01

### 추가

- 유지보수 모드·bypass·명시적 확인을 강제하고 복구 후 실행 세대와 전체 제어면을 재구축하는 `power-cache:restore-finalize`
- 실제 설정·DB 변조, 논리 백업 복구, 과거 Redis 응답 epoch 거부를 검증하는 격리 복구 연습 도구

### 변경

- CI의 지원 G7 트랜잭션 내부 훅을 이동 가능한 브랜치가 아닌 정확한 커밋 `7d628dc4e57153a6217372a8a4bf8ea2904c680f`로 고정
...
- 복구된 아웃박스 재생, 실행 세대 회전, 제어면 초기화, 과거 응답 epoch 격리를 실제 DB·저장소에서 검증
...
- 공식 서비스 데이터 변경·purge·선택적 제어 키 유실·Redis 재시작을 HTTP 부하와 겹쳐 오래된 응답·개인화·복구·정리 상태를 판정하는 장애 캠페인
...
- G7 7.0.10 동일 트랜잭션 데이터 변경 훅 연동과 코어 capability 진단
- 원본 변경, 무효화 아웃박스, 세대 적용의 실제 MySQL·Redis 커밋·롤백 통합 테스트

### 변경

- 페이지·카테고리·상품·게시판·사용자 표현 변경 리스너를 동일 트랜잭션 전용 훅으로 전환
- G7 최소 버전을 7.0.10, 페이지·게시판·이커머스 모듈 최소 버전을 트랜잭션 내부 훅 포함 버전으로 상향
...
- 태그·버전·변경이력 검증, SHA-256 및 빌드 출처 증명가 포함된 릴리스 파이프라인
- 보안 신고, 지원, 기여, 아키텍처 장애 모델, 성능 수용 기준, Beta/1.0 출시 게이트
- doctor의 Redis maxmemory/eviction 정책 및 누적 eviction 진단
- 미적용 아웃박스를 매분 재생하는 자동 reconcile 스케줄

### 수정

- Redis eviction 등으로 세대 키가 사라질 때 `0`으로 되돌아가 과거 응답이 다시 유효해질 수 있던 제어면 결함
- 정상 트랜잭션 롤백 뒤 비상 장벽이 수동 purge 전까지 남던 문제
- 오래된 이벤트 완료가 더 최신 dirty 장벽을 해제할 수 있는 경쟁 조건
- dirty 스냅샷 자동 복구가 유효성 검사 순서 때문에 실행되지 않던 문제
- 콘텐츠가 이미 커밋된 훅에서 장벽 기록 실패가 DB 아웃박스까지 롤백해 복구 근거를 잃던 문제
...
- 장벽·스냅샷·세대 유실 시 실행 세대를 회전하고 전체 알려진 제어 scope를 fail-closed 재구축

## [0.2.0] - 2026-08-23

### 추가

- 비회원 읽기 권한을 원본 미들웨어와 같은 `GuestRoleResolver`로 선검증하는 공개 게시판 목록 캐시
- 페이지 1~3, `per_page` 최대 50의 저카디널리티 상위 목록 허용 정책
...
- 트랜잭션 아웃박스와 장애 복구 장벽
- 정상 HIT의 DB state 조회를 제거하는 전용 저장소 runtime 스냅샷과 커밋 전 emergency 장벽
