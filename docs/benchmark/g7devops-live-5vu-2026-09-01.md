...
세 번의 모드별 구간에서 수집한 서버 전역 누적 계수 차이의 중앙값은 다음과 같다.

| 지표 | BYPASS | ACTIVE | 변화 |
|---|---:|---:|---:|
| PHP-FPM CPU 누적 증가 | 44.820초 | 32.696초 | 27.1% 감소 |
| MySQL Questions 증가 | 6,187 | 3,792 | 38.7% 감소 |
| Redis 명령 수 증가 | 29,703 | 30,711 | 3.4% 증가 |
| 느린 질의 증가 | 0 | 0 | 동일 |

서버 전역 누적 계수에는 같은 시간대의 다른 요청도 포함될 수 있으므로 요청당 절대 사용량이 아니라 방향성 근거로만 사용한다.
...
- 배포: JW PowerCache 0.3.0-beta.1 ACTIVE, G7 트랜잭션 내부 훅 `7d628dc4e57153a6217372a8a4bf8ea2904c680f`
...
이 시험은 의도적으로 총 8 RPS로 고정했으므로 처리량 향상을 증명하지 않는다. 처리량 근거는 별도의 격리 환경 요청 빈도 제한 없는 시험을 사용하고, 실서버에서는 안정성과 정확성 중심으로 해석한다.

## 재현과 원시 증거

원시 JSON, 회차별 판정, 중앙값 요약, 서버 자원 측정값는 [`evidence/2026-09-01-live-5vu`](evidence/2026-09-01-live-5vu/)에 보존한다.
