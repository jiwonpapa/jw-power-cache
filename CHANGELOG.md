...
- 베타 접미사를 제거하고 G7 업데이트 확인이 조회하는 GitHub 일반 Latest 릴리스로 배포
- 플러그인·패키지·잠금 파일 버전 일치 검사와 게시 후 Latest 조회 검사 추가
- 릴리스 업로드 대상을 ZIP·SHA256SUMS 파일로 명시하고, 이후 업데이트 배포 절차 문서화
- 미발행 `0.4.0-beta.2`의 아래 API 캐시 수정 사항을 포함; 기존 호환 범위와 정합성 제한 유지

### Fixed

- JSON·폼·파일 본문이 있는 GET/HEAD를 캐시에서 제외해 게시판 검색·페이지 크기 조건이 일반 목록 캐시에 섞이는 문제 수정
- PHP 기본 시간대 대신 G7 실제 사용자 시간대로 캐시 키를 분리해 다른 시간대의 날짜 응답이 재사용되는 문제 수정
- 예약·CLI outbox 복구가 현재 이벤트 토큰을 확인하고 완료된 장벽까지 해제하도록 수정; 더 최신 이벤트·제어면 재구축 장벽은 보존
- 일반 HIT·락 획득/대기 후 HIT와 저장 직전에 장벽 토큰·runtime epoch·세대를 재확인해 동시 갱신·제어면 재설정 중 이전 응답 재사용 방지

### Changed

- 캐시 정책 키를 `response-api-v4`로 변경해 이전 정책의 응답과 분리

### Tests

- GET JSON 본문과 G7 게시판 입력 파서, 페이지/게시판 시간대 분리, 예약 복구·남은 장벽·새 이벤트 보호, 세 가지 HIT 경로·epoch 재설정·정책 업그레이드 회귀 검사 추가
