...
Loading UX supports the official `sirsoft-basic` and `sirsoft-admin_basic` transition overlays. Four audited large-content spinner profiles are included; action and modal spinners are intentionally excluded. Third-party templates receive overlay-only compatibility until an explicit profile is added. Runtime component registration remains unavailable on tagged G7 releases through 7.0.9 because the required public `G7Core.registerComponents()` API is not implemented there.
